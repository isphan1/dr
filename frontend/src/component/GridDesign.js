...
import {CardContent,Card, Grid, Link, Menu, useMediaQuery } from '@material-ui/core';
...
function ResponsiveDrawer(props) {
  ...
  const drawer = (
    <div>
      <div className={classes.toolbar} style={{display:"flex",}}>
        <Link className={classes.drawerLink}>YOUTUBE</Link>
      </div>
      {/* <Divider /> */}
      <List>
  ...
  return (
    <div className={classes.root}>
      <CssBaseline />
      <AppBar color="inherit" position="fixed" elevation={0} className={classes.appBar}>
        <Toolbar>
          <IconButton
            color="inherit"
      ...
      <nav className={classes.drawer} aria-label="mailbox folders">
        ...
        MenuListProps={xsMatch ? {onClick:handleCloseAnchorEl} : {onMouseLeave:handleCloseAnchorEl}}
      >
        <Grid container style={{outline:"none"}}>
             <Grid item sm={4}> 
            {[1,2,3,4,5].map(item=>(
                // <h3 key={item}>The top gun academy</h3>
                <Card key={item} style={{margin:"10px 0 10px 10px"}}>
                  <CardContent>
